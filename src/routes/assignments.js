...
router.post('/', auth, requireRole(['TEACHER']), [
  ...
  body('type').isIn(['quiz', 'matching', 'drag_and_drop', 'sortable', 'assignment']).withMessage('Valid assessment type is required'),
...
router.patch('/:assignmentId', auth, requireRole(['TEACHER']), [
  ...
  body('description').optional(),
  body('type').optional().isIn(['quiz', 'matching', 'drag_and_drop', 'sortable', 'assignment']).withMessage('Valid assessment type is required'),
  body('category').optional(),
  body('criteria').optional(),
  body('questions').optional(),
  body('dueDate').optional().isISO8601().withMessage('Due date must be a valid date'),
